...
graph TB
    ...
    P -- "子 JSON|{发言人}" --> S1
    S1 -- "读取/解析" --> FS
    S2 -- "AI 评分/分类" --> Claude Code["Claude Code 模型<br/>(Agent 内部)"]
    S2 -- "读取数据" --> FS
    S1 --> FS
    S3 -- "读取数据" --> DB
    S3 -- "输出 Markdown" --> FS
...
flowchart TB
    ...
    subgraph Process["处理流程"]
        P["0. 数据预处理<br/>(preprocessor.py)"]
        A["1. 导入数据<br/>(SKILL.md)"]
        B["2. 解析消息<br/>(SKILL.md)"]
        C["3. 预过滤<br/>(SKILL.md)"]
        D["4. 基础分计算<br/>(SKILL.md)"]
        E["5. AI 评分<br/>(SKILL.md)"]
        F1["6. 聚合统计<br/>(SKILL.md)"]
        G["7. 内容分类<br/>(SKILL.md)"]
        H["8. 数据存储<br/>(storage.py)"]
        I["9. 生成报告<br/>(SKILL.md)"]
    end
