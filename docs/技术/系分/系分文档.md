...
interface AnalyzeInput {
  ...
  whitelist: string[];         // 白名单成员昵称列表
  config?: {
    scoreThreshold?: number;        // 关注分数阈值，默认 60
...
interface AnalyzeOutput {
  ...
  data?: {
    ...
    scoreMembers: ScoreMember[];    // 成员评分名单
  };
  error?: {
    code: string;
    message: string;
...
interface MemberScore {
  ...
  status: "normal" | "score_middle" | "zero_activity" | "low_frequency";
  reason?: string;  // 低质原因
}
...
interface ScoreMember {
  ...
  messageCount: number;
  averageScore: number;
  status: "zero_activity" | "score_middle" | "low_frequency";
...
**实现方式**：由 SKILL.md + report_template.md 定义报告结构与格式
...
| | 5.2 Markdown 输出 | skills/generate-report/report_template.md | 0.25d |
| **Phase 6: 测试** |
